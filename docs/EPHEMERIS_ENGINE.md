...
- **State-vector provenance.** The engine's orbital *elements* are canonical (institutional NASA/JPL sources). The Cartesian position and velocity returned by `computeEphemerisStateVector()` are **derived**, so they are labelled `calculated` with the transformation method recorded, never `canonical`: a state vector computed from a secular model is not itself an institutional measurement. Canonical sandbox initialisation refuses epochs outside the supported domain (`isSupportedEphemerisDay`) instead of presenting extrapolated values under a canonical label.
- **Velocity model.** The state velocity is obtained from the same secular Keplerian model as the position. It is validated against a central finite difference of the full position model across a matrix of bodies (Mercury, Earth, Mars, Jupiter, Ceres, Pluto) at the boundary epochs (1800, J2000, 2026, 2050) in `src/simulation/tests/ephemeris-state-vector.test.ts`, and is treated as an approximation rather than an exact derivative of every secular term. Measured worst case over that 24-entry matrix: **3.4 × 10⁻⁴ of the speed per component and 3.6 × 10⁻⁴ relative speed error**, against a 10⁻³ acceptance bound — so the frozen secular terms are demonstrably below the model's own accuracy.
