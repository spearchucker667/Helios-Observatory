...
export function ogCardPublicPath(cwd = process.cwd()) {
  // Falsy cwd = hermetic mode (unit tests): no workspace filesystem at all.
  if (!cwd) return "";
...
export function normalizeHeadContext(ctx = {}) {
  // Hermetic by default: unit tests call this without `cwd` or `site`, and
  // reading the developer's real workspace (site.json title, public/og.jpg)
  // would leak the app's identity into every assertion. Production paths
  // always pass `cwd` (Vite plugin root) or a baked `site` (middleware
  // snapshot), so their workspace consultation is unchanged.
  const cwd = ctx.cwd ?? null;
  let site;
  if (ctx.site !== undefined) {
    // Middleware passes a baked `site`. With an explicit cwd, re-consult the
    // workspace so a public/og.jpg generated after that snapshot (or missed
    // by a wrong cwd) wins over the og.grok.me placeholder. Vercel passes a
    // cwd without public/, so a correct bake is unchanged there too.
    site = applyCustomCardFromFs(ctx.site, cwd);
  } else if (cwd) {
