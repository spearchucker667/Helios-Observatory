...
// Project root: works both in the Linux sandbox (/workspace) and on a
// developer checkout (repo root derived from this file's location).
...
/** Absolute `target` if it is strictly inside `allowedDirs` (or the project root), else exit 1. */
export function checkedOutputPath(target, allowedDirs, label = "screenshot") {
  ...
  const allowed =
    abs.startsWith(PROJECT_ROOT + sep) ||
    allowedDirs.some((dir) => abs.startsWith(dir.endsWith(sep) ? dir : dir + sep));
  if (!allowed) {
